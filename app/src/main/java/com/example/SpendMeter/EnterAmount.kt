...
import com.example.SpendMeter.databinding.ActivityEnterAmountBinding
...
class EnterAmount : AppCompatActivity() {
    ...
    private val binding: ActivityEnterAmountBinding by lazy {
        ActivityEnterAmountBinding.inflate(layoutInflater)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Set UI text
        binding.userName.text = "You give amount"
        ...
        binding.saveButton.setOnClickListener {
            ...
            val giveRef = databaseReference.child("users").child(currentUser?.uid.toString())
            updateGiveAmount(amount, giveRef)

            val customerGiveRef =
                databaseReference.child("users").child(currentUser?.uid.toString())
                    .child("customer").child(customerId!!)
            updateGiveCustomerAmount(amount, customerGiveRef)
        }
    ...
    fun updateGiveAmount(amount: Int, customerRef: DatabaseReference) {
        customerRef.child("giveAmount").addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                val currentGive = snapshot.getValue(Int::class.java) ?: 0

                // Calculate updated total
                val updatedGive = currentGive + amount

                // Update totalAmount in Firebase
                customerRef.child("giveAmount").setValue(updatedGive)
            }
    ...
    fun updateGiveCustomerAmount(amount: Int, customerRef: DatabaseReference) {
        customerRef.child("giveCustomerAmount")
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    // Update totalAmount in Firebase
                    customerRef.child("giveCustomerAmount").setValue(updatedGive)
                }
        ...
        if (amountKey != null) {
            ...
            customerRef.child(customerId!!).child("amountList").child(amountKey)
                ...
                .addOnSuccessListener {
                    ...
                    startActivity(Intent(this@EnterAmount, CustomerUpdate::class.java))
    ...
    private fun ensureTotalAmountInitialized(
